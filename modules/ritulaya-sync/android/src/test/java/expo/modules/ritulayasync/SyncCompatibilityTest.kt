...
class SyncCompatibilityTest {
    ...
    fun `manifest cannot redirect records to an obsolete or unrelated filename`() {
        val files = SyncRecordsCodec.encode(emptyMap()).toMutableMap()
        files[SyncRecordsCodec.MANIFEST_FILE] =
            JSONObject(files.getValue(SyncRecordsCodec.MANIFEST_FILE))
                .put("files", org.json.JSONArray(listOf("cycles.json", "day-logs.json", "settings.json")))
                .toString()
    ...
    fun `legacy multiline notes and recorded No survive JSON migration`() {
        ...
        val roundTrip = SyncRecordsCodec.decode(SyncRecordsCodec.encode(records))
        ...
        val files = SyncRecordsCodec.encode(unrecorded)
        assertThat(JSONObject(files.getValue(SyncRecordsCodec.MANIFEST_FILE)).getInt("schemaVersion")).isEqualTo(3)
    ...
    fun `fractional protocol versions are rejected without coercion`() {
        val files = SyncRecordsCodec.encode(emptyMap()).toMutableMap()
        files[SyncRecordsCodec.MANIFEST_FILE] =
            JSONObject(files.getValue(SyncRecordsCodec.MANIFEST_FILE)).put("protocolVersion", 2.5).toString()
        SyncRecordsCodec.decode(files)
    }
