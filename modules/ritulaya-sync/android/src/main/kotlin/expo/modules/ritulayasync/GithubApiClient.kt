...
class GithubApiClient(private val token: String) {
    ...
    fun getFileContent(owner: String, repo: String, path: String, branch: String): RepoFile? {
    ...
    fun updateOrCreateFile(
        ...
        content: String, sha: String?, branch: String, message: String
    ) {
    ...
    private fun put(urlStr: String, body: String) {
        val conn = (URL(urlStr).openConnection() as HttpURLConnection).apply {
            requestMethod = "PUT"
            setRequestProperty("Authorization", authHeader())
