...
- 07de684: Update Expo SDK 57 packages to the recommended patch versions and refresh the
  dependency lockfile to restore compatibility checks. Rebuild the Android app to
  include the updated native packages.
- bf56cfa: Show a spinner and localized exporting feedback in Settings while CSV preparation
  and sharing are pending. Disable repeated taps, share one in-flight export across
  callers, and restore the action after completion or failure so users can retry.
- 1fbd371: Make future dates read-only in Today, Calendar and History, with local-date guards
