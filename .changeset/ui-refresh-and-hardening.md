...
- Add on-device entry history with note search and inclusive date, symptom, and
  ...
  in memory and discreet previews hide entry details.
- Refresh Today, Calendar, History, Settings, and the shared editor with consistent
  ...
  without fabricated predictions; calculate cycle days using local calendar dates.
...
- Gate protected routes on successful initialization, remove the unauthenticated
  lock bypass, prevent widget/prediction feedback, and request notification
