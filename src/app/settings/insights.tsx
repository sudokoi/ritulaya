...
export default function InsightsScreen() {
  ...
  return (
    ...
    >
      ...
      {discreet ? (
        <Text className="mx-4 mt-4 text-base text-[var(--text-muted)]">
          {t("discreet.overviewHidden")}
        </Text>
      ) : (
        <>
          ...
          <View className="mx-4 mt-4 rounded-card bg-[var(--bg-surface)] px-5 py-4">
            ...
            {lengthRows.length > 0 ? (
              lengthRows.slice(0, 8).map((row) => (
                <View
                  key={row.startDate}
                  className="flex-row flex-wrap items-center justify-between gap-2 py-1.5"
                >
                  <Text className="text-sm text-[var(--text-primary)]">
                    {format(parseISO(row.startDate), "PP", { locale })}
                  </Text>
                  <View className="flex-row items-center gap-2">
                    {stats &&
                    Math.abs(row.length - stats.median) <= stats.sigma ? null : (
...
function Stat({ value, label }: { value: string; label: string }) {
  return (
    <View className="flex-1 items-center">
      ...
      <Text className="text-center text-xs text-[var(--text-muted)]">{label}</Text>
    </View>
  )
}
