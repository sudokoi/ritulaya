...
- **Sync** — `ritulaya-sync` owns OAuth device flow, revision-based three-way merge, conflict review, atomic Git publication, recovery and WorkManager. The encrypted database stores local revisions and checkpoints. The private repository holds readable root-level `ritulaya-sync-*.json` files, with protocol/schema versions in the manifest. Confirmed migration converts and deletes legacy sync files in one commit, preserving unrelated files and Git history. See [ADR-0012](docs/decisions/0012-revision-based-sync.md).
